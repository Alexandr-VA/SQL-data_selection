...
select singer_name, albums.year from singer
left join singer_albums s_a on s_a.singer_id = singer.id
left join albums on albums.id = s_a.albums_id
where singer_name in (select singer_name from singer_albums where albums.year not between 2020 and 2020)
or singer_name in (select singer_name from singer_albums where albums.year isnull)
